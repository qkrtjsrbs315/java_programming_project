...
public class MultiplayerDodgePanelClient extends ViewPanel {
    ...
    public MultiplayerDodgePanelClient(CollidableObject character, final String imageBomb, final String[] itemImages, GameClient gameClient) {
        super(character);
        this.characterHP = 5;
            ...
            while (!gameEnded) {
                try {
                    ...
                    if (message.startsWith("HP:")) {
                        try {
                            String hpStr = message.substring(3).trim();
                            opponentHP = Integer.parseInt(hpStr);
                            System.out.println("상대 HP: " + opponentHP);
                        } catch (NumberFormatException e) {
                            System.out.println("HP 파싱 오류: " + e.getMessage() + " (값: " + message.substring(3) + ")");
                        }
                    } else if (message.startsWith("GAME_END:")) {
                        try {
                            String hpStr = message.substring(9).trim();
                            System.out.println("파싱 시도: " + hpStr);
                            opponentHP = Integer.parseInt(hpStr);
                            ...
                            System.out.println("클라이언트 결과: " + gameResult);
                            System.out.println("characterHP: " + characterHP + ", opponentHP: " + opponentHP);
                            timer.stop();
                        } catch (NumberFormatException e) {
                            System.out.println("GAME_END 파싱 오류: " + e.getMessage() + " (값: " + message.substring(9) + ")");
                            e.printStackTrace();
                        }
                    }
                } catch (Exception e) {
